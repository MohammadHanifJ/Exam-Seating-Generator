...
app.post("/api/admin/login", async (req, res) => {
  ...
  const result = await query("SELECT * FROM admins WHERE email = $1 AND active = TRUE", [email.toLowerCase()]);
...
app.get("/api/timetables", requireAdmin, async (req, res) => {
  const { department, year, examType } = req.query;
  const filters = [];
  const params = [];
  if (department) {
  ...
  if (examType) {
    ...
    filters.push(`exam_type = $${params.length}`);
  }
  const where = filters.length ? `WHERE ${filters.join(" AND ")}` : "";
  const result = await query(
    `SELECT id, department, year, subject_name, exam_date, start_time, end_time, exam_type
     FROM timetables ${where} ORDER BY exam_date, start_time`,
    params
  );
  res.json(result.rows);
});
...
app.post("/api/timetables", requireAdmin, async (req, res) => {
  const { department, year, subject_name, exam_date, start_time, end_time, exam_type } = req.body || {};
  if (!department || !year || !subject_name || !exam_date || !start_time || !end_time || !exam_type) {
    return res.status(400).json({ error: "All timetable fields are required" });
  }
  const result = await query(
    `INSERT INTO timetables (department, year, subject_name, exam_date, start_time, end_time, exam_type)
     VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`,
    [department, year, subject_name, exam_date, start_time, end_time, exam_type]
  );
  res.json({ ok: true, id: result.rows[0].id });
});

app.put("/api/timetables/:id", requireAdmin, async (req, res) => {
  const { subject_name, exam_date, start_time, end_time, exam_type } = req.body || {};
  await query(
    `UPDATE timetables
     SET subject_name = $1, exam_date = $2, start_time = $3, end_time = $4, exam_type = $5
     WHERE id = $6`,
    [subject_name, exam_date, start_time, end_time, exam_type, req.params.id]
  );
  res.json({ ok: true });
});
...
app.post("/api/upload-students", requireAdmin, (req, res) => {
  // Accept any single file field to avoid field-name mismatches.
  upload.any()(req, res, async (err) => {
    ...
    try {
      ...
      for (const row of valid) {
        const result = await query(
          `INSERT INTO students (name, roll_no, branch, year, approved, blocked) VALUES ($1, $2, $3, $4, FALSE, FALSE)
...
app.post("/api/classrooms", requireAdmin, async (req, res) => {
  ...
  if (!room_no) {
    return res.status(400).json({ error: "room_no is required" });
  }
  await query(
...
app.post("/api/generate", requireAdmin, async (req, res) => {
  ...
  if (examType === "SEMESTER") {
    ...
    const seats = [];
    // Track assigned/unassigned/empty for variable student counts.
...
app.get("/api/seating/:batchId/pdf", requireAdmin, async (req, res) => {
  ...
  Object.keys(grouped).forEach((roomNo) => {
    const assignedSeats = grouped[roomNo].length;
    const assignedStudents = grouped[roomNo].reduce((sum, seat) => sum + (seat.student_two ? 2 : 1), 0);
    const capacity = capacityByRoom[roomNo] || 30;
